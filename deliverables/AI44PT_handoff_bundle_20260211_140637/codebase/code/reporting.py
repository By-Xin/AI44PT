...
def _normalize_type_answer(answer: str) -> str:
    ...
    lower = answer.lower()
    if "split consensus" in lower or "no majority" in lower or re.search(r"\btie\b", lower):
        return "No majority"
    if re.search(r"\buncertain\b", lower):
        return "Uncertain"
    if re.search(r"\bnot\s+applicable\b", lower) or re.search(r"\bn/?a\b", lower):
        return "Not Applicable"

    type_matches = re.findall(r"\btype\s*([1-4])\b", answer, re.IGNORECASE)
    ...
    match = re.match(r"^\s*([1-4])(?:\b|[).:\-])", answer)
    if match:
...
def _normalize_confusion_label(value: object, *, fallback: str) -> str:
    ...
    if normalized == "No majority":
        return "No majority" if fallback == "No majority" else fallback
...
def _determine_article_status(
    ...
    *,
    ...
    tie_condition = (
        ...
        or majority_value_norm.startswith("tie")
        or " tie" in majority_value_norm
