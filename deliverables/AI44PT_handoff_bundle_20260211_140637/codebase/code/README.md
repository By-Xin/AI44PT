...
- **`challenge_reporting.py`** - Dynamic challenge report exporters (Markdown + JSON/CSV/XLSX)
...
# Dynamic challenge workflow: run analysis + immediate mismatch probing + report export
python code/pipeline_main.py --stage challenge

# Optional: specify markdown path for challenge report
python code/pipeline_main.py --stage challenge --challenge-report-path results/challenge_reports/my_report.md
```
...
3. **Dynamic challenge (`--stage challenge`)**
   - Processes articles sequentially for focused case-by-case review
   - Compares each run against the human gold label immediately (Human Q15 vs AI Q16)
   - Triggers an immediate follow-up probe when mismatch or AI unclassified output occurs
   - Exports a human-review report (Markdown + JSON/CSV/XLSX) for expert prompt revision
...
- `challenge_reports/challenge_report_YYYYMMDD_HHMMSS.md` – dynamic mismatch challenge report for expert review.
- `challenge_reports/challenge_events_YYYYMMDD_HHMMSS.(json|csv|xlsx)` – structured challenge diagnostics.
