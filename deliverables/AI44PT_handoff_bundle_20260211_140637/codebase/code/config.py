...
class Config:
    ...
    # 截断策略：超过最大长度后保留前后文，避免仅保留前言导致结论信息丢失
    ARTICLE_TEXT_HEAD_RATIO = 0.6
    ...
    # ==================== Challenge流程配置 ====================
    # 动态 challenge 输出目录（供人工专家复核与改写 prompt）
    CHALLENGE_OUTPUT_DIR = RESULTS_DIR / "challenge_reports"
    # 追问中保留的提示词片段最大长度（字符）
    CHALLENGE_PROMPT_SNIPPET_MAX_CHARS = 12000
    # 追问中保留的原始回答片段最大长度（字符）
    CHALLENGE_RESPONSE_SNIPPET_MAX_CHARS = 12000
    ...
    def setup_directories(cls):
        ...
        cls.CHALLENGE_OUTPUT_DIR.mkdir(exist_ok=True)
    ...
    def validate(cls, stage: str = "full") -> bool:
        ...
        if stage_norm not in {"raw", "parse", "full", "challenge"}:
            logger.warning("Unknown stage '%s'; fallback to full validation", stage)
            stage_norm = "full"

        needs_generation = stage_norm in {"raw", "full", "challenge"}
