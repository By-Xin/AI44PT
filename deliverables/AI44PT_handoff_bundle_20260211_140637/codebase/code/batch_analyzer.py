...
from challenge_reporting import export_challenge_report_bundle
...
class BatchAnalyzer:
    ...
    def _compose_article_text(self, article_pages: List[Dict]) -> str:
        """拼接文章全文；超长时采用前后文保留策略，避免仅保留开头。"""
        ...
        marker = getattr(
            self.config,
            "ARTICLE_TEXT_TRUNCATION_MARKER",
            "\n\n[... MIDDLE CONTENT TRUNCATED ...]\n\n"
        )
        budget = max_chars - len(marker)
        if budget <= 0:
            self.logger.warning(
                "Article text length %s exceeds limit %s; using hard truncation due insufficient marker budget",
                len(full_text),
                max_chars
            )
            return full_text[:max_chars]

        head_ratio = getattr(self.config, "ARTICLE_TEXT_HEAD_RATIO", 0.6)
        if not isinstance(head_ratio, (int, float)):
            head_ratio = 0.6
        head_ratio = min(max(float(head_ratio), 0.1), 0.9)

        head_chars = int(budget * head_ratio)
        tail_chars = budget - head_chars
        truncated_text = f"{full_text[:head_chars].rstrip()}{marker}{full_text[-tail_chars:].lstrip()}"

        self.logger.warning(
            "Article text length %s exceeds limit %s; using head+tail truncation (head=%s, tail=%s)",
            len(full_text),
            max_chars,
            head_chars,
            tail_chars
    ...
    def analyze_single_article(
        ...
        record_stage: str = "raw_generation",
        ...
        # 合并页面内容并在超长时保留前后文，降低“只看到前言”的风险
        article_text = self._compose_article_text(article_pages)
        ...
        record = self._save_raw_response(
            ...
            model_name=current_model, provider=provider, record_stage=record_stage
        )
    ...
    def process_batch(
        ...
        challenge_report_path: Optional[str] = None,
    ):
        """
        ...
        Args:
            ...
            stage: 处理阶段，可选 "raw"、"parse"、"full"、"challenge"
        ...
        if stage == "challenge":
            return self.run_challenge_report(
                excel_path=excel_path,
                raw_output_path=raw_data_path,
                report_output_path=challenge_report_path,
            )
    ...
    def run_challenge_report(
        ...
        动态 challenge 流程：
        - 逐篇执行 AI 分析
        - 每次 run 立即对比人工金标（Human Q15 vs AI Q16）
        - 发生不一致时即时追问诊断 prompt 风险点
        - 导出结构化 challenge 报告供专家人工改写 prompt
        """
        excel_path = excel_path or str(self.config.EXCEL_PATH)
        self.logger.info("📥 Loading Excel for challenge stage: %s", excel_path)
        df_human = pd.read_excel(excel_path)
        total_articles = len(df_human)
        self.logger.info("Found %s articles for challenge stage", total_articles)

        if self.config.DEBUG_MODE:
            self.logger.info("⚙️ Debug mode active: limiting challenge run to first 2 articles")
            df_human = df_human.head(2).copy()
            total_articles = len(df_human)
            self.logger.info("Challenge subset size: %s", total_articles)

        self._ensure_question_columns(df_human)
        question_maps: QuestionMaps = self.config.build_question_maps(df_human.columns)
        human_type_col = question_maps.human.get(15)
        if not human_type_col:
            self.logger.warning(
                "Human Q15 column not detected. Challenge mismatch checks will mark human label as missing."
            )

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        raw_json_path, raw_jsonl_path, run_dir = self._resolve_challenge_raw_paths(raw_output_path, timestamp)
        report_dir, report_md_override = self._resolve_challenge_report_paths(report_output_path)
        raw_jsonl_path.write_text("", encoding="utf-8")
        self.current_raw_run_dir = run_dir

        self.logger.info("📝 Challenge raw JSON output: %s", raw_json_path)
        self.logger.info("🧮 Challenge raw JSONL stream: %s", raw_jsonl_path)
        self.logger.info("📄 Challenge report directory: %s", report_dir)

        self._ensure_clients_initialized()
        providers = self.available_providers or [self.config.LLM_PROVIDER]
        ai_runs = self.config.get_ai_runs()
        ...
        challenge_iter = df_human.iterrows()
        if tqdm:
            challenge_iter = tqdm(
                challenge_iter,
                total=total_articles,
                desc="Challenge analysis",
                unit="article",
            )

        for seq_idx, (_, row) in enumerate(challenge_iter, start=1):
            article_id = str(row.get('#', ''))
            title = str(row.get('Title of the Paper', 'Unknown') or 'Unknown')
            human_label = row.get(human_type_col, '') if human_type_col else ''
            if human_label is None or (isinstance(human_label, float) and pd.isna(human_label)):
                human_label_text = ""
            else:
                human_label_text = str(human_label)
            human_label_normalized = self._normalize_type_for_challenge(human_label)

            self.logger.info(
                "[%s/%s] Challenge article #%s: %s...",
                seq_idx,
                total_articles,
                article_id,
                title[:60],
            ...
            pdf_path = self._find_pdf(article_id)
            if not pdf_path:
                self.logger.warning("  ⚠️ PDF not found for article #%s", article_id)
                for provider in providers:
                    model_name = self.config.GEMINI_MODEL if provider == "gemini" else self.config.CLS_MODEL
                    article_meta = dict(article_meta_base)
                    article_meta["provider"] = provider
                    for run_index in range(1, ai_runs + 1):
                        err_record = self._save_raw_response(
                            ...
                            model_name=model_name,
                            provider=provider,
                            record_stage="challenge_generation",
                        )
                        raw_records.append(err_record)
                        self._append_jsonl_record(raw_jsonl_path, err_record)
                        challenge_rows.append(
                            {
                                ...
                                "provider": provider,
                                "run_index": run_index,
                                "analysis_timestamp": err_record.get("timestamp", ""),
                                "analysis_status": "error",
                                "analysis_error_type": "PDF_NOT_FOUND",
                                "analysis_error": "PDF not found",
                                "human_label": human_label_text,
                                "human_label_normalized": human_label_normalized,
                                "ai_label": "",
                                "ai_label_normalized": "",
                                "comparison_status": "pdf_not_found",
                                "probe_triggered": False,
                                "probe_status": "not_triggered",
                                "probe_timestamp": "",
                                "probe_summary": "",
                                "root_cause": "",
            ...
            article_meta_base["pdf_path"] = pdf_path
            try:
                article_pages = self.document_reader.read_pdf(pdf_path)
            except Exception as exc:
                self.logger.warning("  ⚠️ Exception while reading PDF: %s", exc)
                article_pages = []

            if not article_pages:
                self.logger.warning("  ⚠️ PDF read failed for article #%s", article_id)
                for provider in providers:
                    model_name = self.config.GEMINI_MODEL if provider == "gemini" else self.config.CLS_MODEL
                    article_meta = dict(article_meta_base)
                    article_meta["provider"] = provider
                        ...
                        challenge_rows.append(
                            {
                                ...
                                "analysis_timestamp": err_record.get("timestamp", ""),
                                "analysis_status": "error",
                                "analysis_error_type": "PDF_READ_ERROR",
                                "analysis_error": "PDF read error",
                                "human_label": human_label_text,
                                "human_label_normalized": human_label_normalized,
                                "ai_label": "",
                                "ai_label_normalized": "",
                                "comparison_status": "pdf_read_error",
                                "probe_triggered": False,
                                "probe_status": "not_triggered",
                                "probe_timestamp": "",
                                "probe_summary": "",
            ...
            for provider in providers:
                client = self.clients.get(provider)
                if not client:
                    self.logger.warning("  ⚠️ Provider client unavailable: %s", provider)
                    model_name = self.config.GEMINI_MODEL if provider == "gemini" else self.config.CLS_MODEL
                    article_meta = dict(article_meta_base)
                    article_meta["provider"] = provider
                    for run_index in range(1, ai_runs + 1):
                        err_record = self._save_raw_response(
                            ...
                            api_timestamp=self._get_timestamp(),
                            status="error",
                            error_message=f"Provider client not initialized: {provider}",
                        ...
                        challenge_rows.append(
                            {
                                ...
                                "analysis_timestamp": err_record.get("timestamp", ""),
                                "analysis_status": "error",
                                "analysis_error_type": "CLIENT_NOT_INITIALIZED",
                                "analysis_error": f"Provider client not initialized: {provider}",
                                "human_label": human_label_text,
                                "human_label_normalized": human_label_normalized,
                                "ai_label": "",
                                "ai_label_normalized": "",
                                "comparison_status": "analysis_error",
                                "probe_triggered": False,
                                "probe_status": "not_triggered",
                                "probe_timestamp": "",
                                "probe_summary": "",
                ...
                for run_index in range(1, ai_runs + 1):
                    ...
                    response_text, api_timestamp, record = self.analyze_single_article(
                        article_pages=article_pages,
                        coding_task_pages=coding_task_pages,
                        executive_summary_pages=executive_summary_pages,
                    ...
                    raw_records.append(record)
                    self._append_jsonl_record(raw_jsonl_path, record)
                    ...
                    row_out: Dict[str, Any] = {
                        ...
                        "provider": provider,
                        "run_index": run_index,
                        "analysis_timestamp": api_timestamp,
                        "analysis_status": record.get("status", "error"),
                        "analysis_error_type": record.get("error_type", ""),
                        "analysis_error": record.get("error", ""),
                        "human_label": human_label_text,
                        "human_label_normalized": human_label_normalized,
                        "ai_label": "",
                        "ai_label_normalized": "",
                    ...
                    if not response_text:
                        challenge_rows.append(row_out)
                        continue

                    parsed_answers = self.parser.parse_response(response_text)
                    if not parsed_answers:
                        row_out["comparison_status"] = "parse_error"
                        row_out["analysis_error_type"] = row_out["analysis_error_type"] or "PARSE_ERROR"
                        challenge_rows.append(row_out)
                        continue

                    ai_label = parsed_answers.get(self.config.Q_ID_CLASSIFICATION, "")
                    ai_label_normalized = self._normalize_type_for_challenge(ai_label)
                    comparison_status = self._classify_challenge_comparison(
                        human_label_normalized=human_label_normalized,
                        ai_label_raw=str(ai_label or ""),
                        ai_label_normalized=ai_label_normalized,
                    )

                    row_out["ai_label"] = str(ai_label or "")
                    row_out["ai_label_normalized"] = ai_label_normalized
                    row_out["comparison_status"] = comparison_status

                    if comparison_status in {"mismatch", "ai_unclassified"}:
                        probe_out = self._run_challenge_probe(
                            client=client,
                            provider=provider,
                            model_name=model_name,
                            article_meta=article_meta,
                            ...
                            human_label=human_label_text,
                            human_label_normalized=human_label_normalized,
                            ai_label=str(ai_label or ""),
                            ai_label_normalized=ai_label_normalized,
                            analysis_prompt=str(record.get("prompt") or ""),
                            analysis_response=str(response_text or ""),
                            parsed_answers=parsed_answers,
                        )
                        row_out["probe_triggered"] = True
                        row_out["probe_status"] = probe_out.get("probe_status", "error")
                        row_out["probe_timestamp"] = probe_out.get("probe_timestamp", "")
                        row_out["probe_summary"] = probe_out.get("probe_summary", "")
                        row_out["root_cause"] = probe_out.get("root_cause", "")
                        row_out["prompt_risk_sentence"] = probe_out.get("prompt_risk_sentence", "")
                        row_out["minimal_prompt_fix"] = probe_out.get("minimal_prompt_fix", "")
                        row_out["expert_review_checklist"] = probe_out.get("expert_review_checklist", "")

                        probe_record = probe_out.get("probe_record")
                        if isinstance(probe_record, dict):
                            raw_records.append(probe_record)
                            self._append_jsonl_record(raw_jsonl_path, probe_record)

                    challenge_rows.append(row_out)

        with open(raw_json_path, 'w', encoding='utf-8') as json_file:
            json.dump(raw_records, json_file, ensure_ascii=False, indent=2)

        report_paths = export_challenge_report_bundle(
            challenge_rows=challenge_rows,
            output_dir=report_dir,
            timestamp=timestamp,
            report_markdown_path=report_md_override,
        )
        report_paths["raw_json_path"] = raw_json_path
        report_paths["raw_jsonl_path"] = raw_jsonl_path

        issue_rows = [
            row for row in challenge_rows if row.get("comparison_status") in {"mismatch", "ai_unclassified"}
        ]
        self.logger.info(
            "📊 Challenge summary: %s run rows, %s issue rows requiring expert review",
            len(challenge_rows),
            len(issue_rows),
        )
        self.logger.info("📄 Challenge markdown report: %s", report_paths.get("report_markdown"))
        self.logger.info("🧾 Challenge structured data: %s", report_paths.get("report_json"))

        self.last_raw_source_path = raw_json_path
        self.current_raw_run_dir = None
        return report_paths

    def _resolve_challenge_raw_paths(
        self,
        raw_output_path: Optional[str],
        timestamp: str,
    ) -> Tuple[Path, Path, Path]:
        """解析 challenge 阶段的 raw 输出路径。"""
        if raw_output_path:
            target = Path(raw_output_path).expanduser()
            if target.suffix:
                base_dir = target.parent
                run_dir = base_dir / target.stem
                output_name = target.with_suffix(".json").name
            else:
                base_dir = target
                run_dir = base_dir / f"challenge_batch_{timestamp}"
                output_name = f"challenge_raw_{timestamp}.json"
        else:
            base_dir = self.config.RAW_OUTPUT_DIR
            run_dir = base_dir / f"challenge_batch_{timestamp}"
            output_name = f"challenge_raw_{timestamp}.json"
        ...
        run_dir.mkdir(parents=True, exist_ok=True)
        raw_json_path = run_dir / output_name
        raw_jsonl_path = raw_json_path.with_suffix(".jsonl")
        return raw_json_path, raw_jsonl_path, run_dir

    def _resolve_challenge_report_paths(
        self,
        report_output_path: Optional[str],
    ) -> Tuple[Path, Optional[Path]]:
        """
        解析 challenge 报告导出目录与 markdown 覆盖路径。
        - 若提供 .md 文件路径：使用该文件作为 markdown 输出
        - 若提供目录：按默认命名输出 markdown/json/csv/xlsx
        """
        if not report_output_path:
            report_dir = Path(self.config.CHALLENGE_OUTPUT_DIR)
            report_dir.mkdir(parents=True, exist_ok=True)
            return report_dir, None

        target = Path(report_output_path).expanduser()
    ...
    def _append_jsonl_record(self, path: Path, record: Dict[str, Any]):
        """向 challenge JSONL 流式文件追加记录。"""
    ...
    def _extract_instruction_snippet(self, full_prompt: str) -> str:
        """从完整提示中提取与 prompt 设计相关的片段，避免追问上下文过长。"""
        if not full_prompt:
            return ""

        snippets: List[str] = []
        guidelines_match = re.search(
            r"(### General Guidelines.*?)(?:<BEGIN_4PT_RESPONSE>|### Reference Materials)",
            full_prompt,
            re.IGNORECASE | re.DOTALL,
        )
        if guidelines_match:
            snippets.append(guidelines_match.group(1).strip())

        reminders_match = re.search(
            r"(### CRITICAL REMINDERS.*?)(?:### Article to Analyze)",
            full_prompt,
            re.IGNORECASE | re.DOTALL,
        )
        if reminders_match:
            snippets.append(reminders_match.group(1).strip())

        key_question_lines = []
        for line in full_prompt.splitlines():
            stripped = line.strip()
            if re.match(r"Q(15|16|17)\.\s", stripped):
                key_question_lines.append(stripped)
        if key_question_lines:
            snippets.append("Key classification questions:\n" + "\n".join(key_question_lines))

        if not snippets:
            snippets = [full_prompt]

        joined = "\n\n".join(snippets)
        return self._truncate_text(joined, self.config.CHALLENGE_PROMPT_SNIPPET_MAX_CHARS)

    @staticmethod
    def _parse_challenge_probe_response(response_text: str) -> Dict[str, str]:
        """解析 challenge 追问结果（固定 XML 标签）。"""
        if not response_text:
            return {}

        tags = [
            ("DISAGREEMENT_SUMMARY", "probe_summary"),
            ("ROOT_CAUSE", "root_cause"),
            ("PROMPT_RISK_SENTENCE", "prompt_risk_sentence"),
            ("MINIMAL_PROMPT_FIX", "minimal_prompt_fix"),
            ("EXPERT_REVIEW_CHECKLIST", "expert_review_checklist"),
        ]
        parsed: Dict[str, str] = {}
        for tag_name, field_name in tags:
            ...
            parsed[field_name] = match.group(1).strip() if match else ""
        return parsed

    def _normalize_type_for_challenge(self, value: Any) -> str:
        """标准化 type 标签，仅保留 Type 1-4。"""
        ...
        if isinstance(normalized, str) and re.fullmatch(r"Type [1-4]", normalized.strip()):
            return normalized.strip()
        return ""

    @staticmethod
    def _classify_challenge_comparison(
        human_label_normalized: str,
        ai_label_raw: str,
        ai_label_normalized: str,
    ) -> str:
        """给 challenge 运行结果打上对比状态标签。"""
        if not human_label_normalized:
            return "human_missing"
        if not str(ai_label_raw or "").strip():
            return "ai_missing"
        if not ai_label_normalized:
            return "ai_unclassified"
        if human_label_normalized == ai_label_normalized:
            return "match"
        return "mismatch"

    @staticmethod
    def _build_key_answer_snapshot(answers: Dict[int, str]) -> str:
        """构建用于追问的关键问题快照。"""
        key_questions = [3, 6, 9, 12, 15, 16, 17, 18, 20, 22, 24]
        lines = []
        for q_num in key_questions:
            value = str(answers.get(q_num, "") or "").strip()
            if value:
                lines.append(f"Q{q_num}: {value}")
        return "\n".join(lines) if lines else "[No key answers parsed]"

    def _build_challenge_probe_prompts(
        self,
        article_id: str,
        provider: str,
        run_index: int,
        ...
        """构建 challenge 追问提示词。"""
        system_prompt = (
            "You are auditing a 4PT classification prompt. "
            "Your job is to diagnose why the model output diverged from the gold label, "
            "without forcing agreement. Focus on prompt ambiguity, instruction conflicts, "
            "and missing guardrails."
        )

        instruction_snippet = self._extract_instruction_snippet(analysis_prompt)
        response_snippet = self._truncate_text(
            analysis_response or "",
            self.config.CHALLENGE_RESPONSE_SNIPPET_MAX_CHARS,
        )
        key_answers = self._build_key_answer_snapshot(parsed_answers)

        user_prompt = f"""
Audit this single challenge case and return only the XML template.
...
Gold standard:
- Human label (raw): {human_label}
- Human label (normalized): {human_label_normalized or "[missing]"}

AI output:
- AI label (raw): {ai_label}
- AI label (normalized): {ai_label_normalized or "[unclassified]"}

Key parsed answers from this run:
{key_answers}
...
Return strictly in this XML format:
<BEGIN_CHALLENGE_PROBE>
<DISAGREEMENT_SUMMARY>What exactly disagrees and why this matters for coding reliability.</DISAGREEMENT_SUMMARY>
<ROOT_CAUSE>Most likely root cause(s), with emphasis on prompt wording or instruction interaction.</ROOT_CAUSE>
<PROMPT_RISK_SENTENCE>Which instruction sentence/pattern is risky or ambiguous.</PROMPT_RISK_SENTENCE>
<MINIMAL_PROMPT_FIX>One minimal prompt rewrite proposal for human experts to review.</MINIMAL_PROMPT_FIX>
<EXPERT_REVIEW_CHECKLIST>3 concrete checks for expert coders before accepting this fix.</EXPERT_REVIEW_CHECKLIST>
</END_CHALLENGE_PROBE>
""".strip()
        return system_prompt, user_prompt

    def _run_challenge_probe(
        self,
        *,
        client,
        provider: str,
        ...
        """执行单次 challenge 追问并返回结构化结果。"""
        system_prompt, user_prompt = self._build_challenge_probe_prompts(
            article_id=str(article_meta.get("article_id", "")),
            provider=provider,
            run_index=run_index,
            comparison_status=comparison_status,
        ...
        try:
            probe_response = client.generate_response(
                system_prompt=system_prompt,
                user_prompt=user_prompt,
                temperature=self.config.TEMPERATURE,
                reasoning_effort=self.config.get_reasoning_effort(),
            ...
            probe_record = self._save_raw_response(
                article_meta=article_meta,
                run_index=run_index,
                prompt=full_prompt,
                response_text=probe_response,
                api_timestamp=probe_timestamp,
                status="success",
                error_message=None,
                error_type=None,
                model_name=model_name,
                provider=provider,
                record_stage="challenge_probe",
            )
            parsed_probe = self._parse_challenge_probe_response(probe_response)
            return {
                "probe_status": "success",
                "probe_timestamp": probe_timestamp,
                "probe_summary": parsed_probe.get("probe_summary", ""),
                "root_cause": parsed_probe.get("root_cause", ""),
                "prompt_risk_sentence": parsed_probe.get("prompt_risk_sentence", ""),
                "minimal_prompt_fix": parsed_probe.get("minimal_prompt_fix", ""),
                "expert_review_checklist": parsed_probe.get("expert_review_checklist", ""),
            ...
            probe_record = self._save_raw_response(
                ...
                status="error",
                error_message=str(exc),
                error_type="challenge_probe_error",
                model_name=model_name,
                provider=provider,
                record_stage="challenge_probe",
            )
            return {
                "probe_status": "error",
                "probe_timestamp": probe_timestamp,
                "probe_summary": f"Probe failed: {exc}",
                "root_cause": "",
                "prompt_risk_sentence": "",
                "minimal_prompt_fix": "",
                "expert_review_checklist": "",
