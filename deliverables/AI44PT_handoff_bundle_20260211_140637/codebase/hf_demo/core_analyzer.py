...
class Core4PTAnalyzer:
    ...
    def __init__(self, api_key: str):
        ...
        self.max_article_chars = 120000
        self.article_head_ratio = 0.6
    ...
    def _load_codebook(self) -> str:
        ...
        candidate_paths = [
            repo_root / "data" / "instructions" / "TheCodingTask.md",
            repo_root / "data" / "instructions" / "ExecutiveSummary.md",
            repo_root / "data" / "instructions" / "MainBody.md",
            Path(__file__).resolve().parent / "code" / "data" / "processed" / "TheCodingTask.md",
        ]
        ...
        for doc_path in candidate_paths:
            ...
            try:
                ...
                continue
            if content:
                documents.append(f"### {doc_path.name}\n{content}")
...
"""
    ...
    def _create_analysis_prompt(self, paper_text: str) -> str:
        ...
        paper_text = self._truncate_article_text(paper_text)
...
"""
    ...
    def _truncate_article_text(self, paper_text: str) -> str:
        """Keep head+tail context when the paper text exceeds the budget."""
        if len(paper_text) <= self.max_article_chars:
            return paper_text

        marker = "\n\n[... MIDDLE CONTENT TRUNCATED ...]\n\n"
        budget = self.max_article_chars - len(marker)
        if budget <= 0:
            return paper_text[:self.max_article_chars]

        head_ratio = min(max(float(self.article_head_ratio), 0.1), 0.9)
        head_chars = int(budget * head_ratio)
        tail_chars = budget - head_chars
        return f"{paper_text[:head_chars].rstrip()}{marker}{paper_text[-tail_chars:].lstrip()}"
